...
//  https://github.com/nathanaltice/SecretoftheOoze/blob/master/src/Ooze.js
//
class Ooze extends Phaser.Scene
{
    ...
    create()
    {
        ...
        // create state machine on ooze object, passing JSON
        // states object & target object
        this.ooze.oozeFSM = new StateMachine(this.oozeStates, this.ooze);

        // initialize our transition flag
        ...
        (
            ...
            `State: ${this.ooze.oozeFSM.getState().name}`
        ).setOrigin(0.5);

        this.transitionText = this.add.text
        (
            game.config.width / 2,
            game.config.height / 6 * 5,
            `Transition: stable`
        ).setOrigin(0.5);
        ...
        document.getElementById("info").innerHTML =
            "<strong>ooze.js</strong>: Left Arrow (Cool) / Right Arrow (Heat) / Other Arrows (???)";
